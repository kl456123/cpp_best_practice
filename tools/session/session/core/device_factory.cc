...
// static
int32_t DeviceFactory::DevicePriority(const string& device_type) {
...
Status DeviceFactory::ListAllPhysicalDevices(std::vector<string>* devices) {
    ...
    RETURN_IF_ERROR(cpu_factory->ListPhysicalDevices(devices));
    ...
    for (auto& p : device_factories()) {
        ...
        if (factory != cpu_factory) {
            RETURN_IF_ERROR(factory->ListPhysicalDevices(devices));
        }
    }
...
std::unique_ptr<Device> DeviceFactory::NewDevice(const string& type,
    ...
    CHECK_OK(device_factory->CreateDevices(opt, name_prefix, &devices));
    int expected_num_devices = 1;
    auto iter = options.config.device_count().find(type);
    if (iter != options.config.device_count().end()) {
        expected_num_devices = iter->second;
    }
    CHECK_EQ(devices.size(), static_cast<size_t>(expected_num_devices));
    return std::move(devices[0]);
}

Status DeviceFactory::AddDevices(const SessionOptions& options, const string& name_prefix,
        std::vector<std::unique_ptr<Device>>* devices){
    // CPU first. A CPU device is required.
    auto cpu_factory = GetFactory("CPU");
    if (!cpu_factory) {
    ...
    size_t init_size = devices->size();
    RETURN_IF_ERROR(cpu_factory->CreateDevices(options, name_prefix, devices));
    if (devices->size() == init_size) {
        return errors::NotFound("No CPU devices are available in this process");
    }
    ...
    for (auto& p : device_factories()) {
        auto factory = p.second.factory.get();
        if (factory != cpu_factory) {
            RETURN_IF_ERROR(factory->CreateDevices(options, name_prefix, devices));
        }
    }
