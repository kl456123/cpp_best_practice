...
void main(){
    ...
    for(int ind=0;ind<4;++ind){
        ...
        if(input_coord[3]%4==0){
            res[ind] = texelFetch(input_image, ivec2(offset, 0), 0).x;
        }else if(input_coord[3]%4==1){
            res[ind] = texelFetch(input_image, ivec2(offset, 0), 0).y;
        }else if(input_coord[3]%4==2){
            res[ind] = texelFetch(input_image, ivec2(offset, 0), 0).z;
        }else if(input_coord[3]%4==3){
            res[ind] = texelFetch(input_image, ivec2(offset, 0), 0).w;
        }
